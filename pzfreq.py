...
def read_pzem_004t():
    ...
    if len(response) == 25 and response[0] == 0xF8 and response[1] == 0x04:
        ...
        frequency = freq_raw / 10.0 if freq_raw != 0 else "Not available"
        ...
        calculated_pf = power / (voltage * current) if voltage * current != 0 else 0
    ...
    try:
        while True:
            ...
            if data:
                ...
                print(f"Frequency: {data['frequency']} Hz (Raw: {data['frequency_raw']})")
                print(f"Power Factor (device): {data['power_factor']:.3f}")
                print(f"Power Factor (raw): {data['power_factor_raw']}")
                print(f"Power Factor (calculated): {data['calculated_pf']:.3f}")
                print("-------------------------")
            else:
